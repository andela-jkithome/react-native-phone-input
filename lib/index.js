...
export default class PhoneInput extends Component {
  ...
  render() {
    const { iso2, formattedNumber, disabled, ghostText } = this.state;
    const TextComponent = this.props.textComponent || TextInput;
    const { style, flagStyle, offset, textStyle, textStyle: { fontSize}, textProps } = this.props;
    let paddingLeft = fontSize > 12 ? 25 : 20;
    const ghostStyle = { ...textStyle, color: "#6D6D6D", paddingLeft };
    delete ghostStyle.padding;
      ...
      <View style={[styles.container, style]}>
        ...
        <View style={{ flex: 1, marginLeft: offset || 10 }}>
          ...
          <TextComponent
            ...
            editable={!disabled}
            autoCorrect={false}
            style={[styles.text, textStyle, { position: "absolute", width: "100%" }]}
